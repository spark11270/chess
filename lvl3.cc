...
pair<pair<int, int>, pair<int, int>> LVL3::getAIMove(Board *board) {
  vector<pair<shared_ptr<Piece>, pair<int, int>>> myMovePairs = board->getAllValidMoves(board->isWhiteTurn());
  vector<pair<shared_ptr<Piece>, pair<int, int>>> opponentMovePairs = board->getAllValidMoves(!board->isWhiteTurn());
